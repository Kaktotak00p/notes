...
<div class="flex flex-col justify-start w-full h-full gap-4 pb-8 overflow-y-auto">
	...
	<div class="flex flex-col justify-start px-8 py-6">
		...
		<h1 class="text-2xl font-semibold">{$profile?.full_name ?? data.session.user.email}'s Notes</h1>
	</div>
	...
	<div class="flex flex-col justify-start w-full gap-12 px-8">
		...
		<div class="flex flex-col gap-4">
			...
			<div class="flex flex-row flex-wrap justify-between w-full gap-4">
				<!-- Reading List -->
				<div class="flex flex-row items-center gap-6 p-2 rounded-md">
					<!-- Icon -->
					...
					<div class="flex flex-col items-start gap-1">
						...
						<button
							...
							on:click={() => addNote('Reading List')}>Create note</button
						>
					</div>
				</div>
				...
				<div class="flex flex-row items-center gap-6 p-2 rounded-md">
					...
					<div class="flex flex-col items-start gap-1">
						...
						<button class="text-sm font-semibold text-primary" on:click={() => addNote('Recipe')}
							>Create note</button
						>
					</div>
				</div>
				...
				<div class="flex flex-row items-center gap-6 p-2 rounded-md">
					...
					<div class="flex flex-col items-start gap-1">
						...
						<button class="text-sm font-semibold text-primary" on:click={() => addNote('Idea')}
							>Create note</button
						>
					</div>
				</div>
		...
		<div class="flex flex-col justify-start">
			...
			{#if $notes.filter((note) => !note.deleted).length > 0}
				<div class="flex flex-row flex-wrap gap-4">
					{#each $notes
						...
						.slice(0, 4) as note}
						<button
							class="flex flex-col items-start justify-between w-48 h-64 gap-2 p-4 transition-transform duration-200 ease-in-out bg-white border rounded-md hover:scale-105"
							on:click={() => {
							...
							<div class="flex flex-col items-start gap-2">
								...
								{#if note.categoryid}
									{#each $categories.filter((category) => category.id === note.categoryid) as category}
										<Badge variant="outline" class="text-gray-500 rounded-md"
					...
					<button
						class="flex flex-row items-center justify-center w-48 h-64 p-4 transition-transform duration-200 ease-in-out border border-gray-300 border-dashed rounded-md hover:scale-105"
						on:click={() => addNote('New Note')}
					>
			...
			{:else}
				<p class="text-sm text-gray-500">
					No notes yet. <a href="/private/notes" class="text-primary hover:underline"
						>Add a note to get started.</a
					>
				</p>
			{/if}
		</div>
		...
		<div class="flex flex-col justify-start">
			...
			{#if $tasks.filter((task) => !task.aiGenerated).length > 0}
				<div class="flex flex-row flex-wrap gap-4">
					...
					<button class="text-sm font-semibold text-primary" on:click={() => goto('/private/tasks')}
						>View all tasks</button
					>
				</div>
			{:else}
				<p class="text-sm text-gray-500">
					No tasks yet. <a href="/private/tasks" class="text-primary hover:underline"
						>Add a task to get started.</a
					>
				</p>
			{/if}
		</div>
